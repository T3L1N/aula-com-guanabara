...
function calcular() {
    ...
    var igual = document.querySelector('input#igual')
    ...
    if (numa.value.length == 0 || numa.value == 0 || numb.value.length == 0 || numb.value == 0 || numc.value.length == 0 || numc.value == 0) {
        ...
        var ig = Number(igual.value)
        var delta = nb **2 - 4 * na * nc

        res.innerHTML = `<p>A equação atual é ${na}x<sup>2</sup> + ${formatarNumero(nb)}x + ${formatarNumero(nc)} = ${ig}</p>`
        res.innerHTML += `<p>a fórmula de Δ é: ${formatarNumero(nb)}<sup>2</sup> - 4 • ${formatarNumero(na)} • ${formatarNumero(nc)}</p>`
